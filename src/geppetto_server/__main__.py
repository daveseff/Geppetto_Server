...
from .pki import clean_agent_cert, ensure_server_pki, list_certs, sign_agent_cert
...
def _parse_args(argv: list[str] | None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Geppetto config server")
    subparsers = parser.add_subparsers(dest="command")

    subparsers.add_parser("serve", help="Run the HTTPS config server")
    subparsers.add_parser("init", help="Initialize CA and server certificates")

    cert = subparsers.add_parser("cert", help="Manage agent certificates")
    cert_subparsers = cert.add_subparsers(dest="cert_command", required=True)
    cert_subparsers.add_parser("list", help="List pending CSRs and signed certificates")
    sign = cert_subparsers.add_parser("sign", help="Sign a pending agent CSR")
    sign.add_argument("host")
    clean = cert_subparsers.add_parser("clean", help="Remove a pending CSR and signed cert")
    clean.add_argument("host")
    status = cert_subparsers.add_parser("status", help="Show certificate state for one host")
